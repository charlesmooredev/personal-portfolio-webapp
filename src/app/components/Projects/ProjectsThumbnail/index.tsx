  ...
  return (
    <NavLink to={data.link} className={projectCls}>
      <div className="w-full overflow-hidden rounded-[12px] border-2 border-black-900 lg:relative">
        <div className="lg:group-hover:block lg:absolute text-[1.15rem] hidden bg-black-500/50 z-[999] backdrop-blur left-0 bottom-0 w-full p-[10px]">
          {data.description}
        </div>
        <img
          alt={data.thumbnail}
      ...
      <div className="lg:bg-transparent lg:space-y-0 space-y-[5px] bg-black-900 rounded-[10px] p-[5px]">
        ...
        <div className="lg:hidden text-black-100 text-[1.10rem]">
          {data.description}
        </div>
      </div>
    </NavLink>
